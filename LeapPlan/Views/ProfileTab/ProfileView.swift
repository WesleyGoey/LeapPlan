                ...
                VStack {
                    if viewModel.isLoading {
                            ...
                            VStack(spacing: 24) {
                                VStack(spacing: 16) {
                                    ZStack(alignment: .bottomTrailing) {
                                        if let base64String = user
                                            .profileImageUrl,
                                            let uiImage = Base64Helper.decode(
                                                base64String
                                            )
                                        {
                                            ...
                                            Circle().fill(
                                                Color.leapPrimary.opacity(0.2)
                                            )
                                            .frame(width: 110, height: 110)
                                            .overlay(
                                                Text(
                                                    String(
                                                        user.fullName.prefix(1)
                                                    ).uppercased()
                                                ).font(
                                                    .system(
                                                        size: 45,
                        ...
                        VStack(spacing: 20) {
                            ...
                            ).font(.system(size: 80)).foregroundColor(
                                .gray.opacity(0.5)
                            )
                            Text("You're not logged in").font(.title2.bold())
                                .foregroundColor(.leapSecondary)
                            Text(
                                "Login or create an account to start planning your perfect trip, saving itineraries, and more."
                            ...
                            Button(action: {
                                ...
                                Text("Login / Register").fontWeight(.semibold)
        ...
        NavigationStack {
            VStack(spacing: 24) {
                ...
                if let error = viewModel.errorMessage {
                    Text(error).font(.caption).foregroundColor(.white).padding()
                        .background(Color.leapHighlight.opacity(0.9))
                ...
                .disabled(
                    viewModel.authEmail.isEmpty
                        || viewModel.authPassword.isEmpty
                        || (!isLoginMode && viewModel.authFullName.isEmpty)
                )
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Close") { dismiss() }.foregroundColor(
                        .leapSecondary
                    )
                }
            }
