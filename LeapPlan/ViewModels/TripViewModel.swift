...
class TripViewModel: ObservableObject {
    ...
    private func setupGenerateFormLiveSearch() {
        $destinationForm
            ...
            .sink { [weak self] query in self?.performFormSearch(query: query) }
    ...
    private func performFormSearch(query: String) {
        ...
        Task {
            do {
                ...
                print(
                    "Foursquare Autocomplete Error: \(error.localizedDescription)"
                )
            }
        }
    }
    ...
    // MARK: - ADD PLACE TO ITINERARY (Dari Explore Tab)
    func addPlaceToTrip(
        place: FSQPlace,
        targetTrip: Trip,
