    ...
    var body: some View {
        ZStack(alignment: .top) {
            ...
            Map(position: $viewModel.cameraPosition, selection: $selectedMapFeature) {
                UserAnnotation {
                    ZStack {
                        Circle().fill(Color.leapPrimary.opacity(0.2)).frame(width: 32, height: 32)
                        Circle().stroke(Color.white, lineWidth: 2).background(Circle().fill(Color.leapPrimary)).frame(width: 14, height: 14)
                    }
                }
                                ...
                                VStack(alignment: .leading, spacing: 0) {
                                    ForEach(viewModel.searchResults) { place in
                                        Button(action: {
                                            ...
                                            HStack(spacing: 15) {
                                                // REVISI: MUNCULKAN GAMBAR ASLI DI LIST SEARCH
                                                if let imgUrl = place.imageURL, let url = URL(string: imgUrl) {
                                                    AsyncImage(url: url) { phase in
                                                        if let image = phase.image {
                                                            image.resizable().scaledToFill().frame(width: 40, height: 40).clipShape(RoundedRectangle(cornerRadius: 8))
                                                        } else {
                ...
                if !isSearching {
                    HStack {
                        ...
                        Button(action: { withAnimation { viewModel.centerToCurrentLocation() } }) {
                            Image(systemName: "location.fill").font(.system(size: 20)).foregroundColor(Color.leapPrimary)
        ...
        .sheet(item: $viewModel.selectedPlace) { place in
            // LEMPAR BINDING PLACE_TO_ADD AGAR DATA TIDAK HILANG SAAT SHEET DITUTUP
            PlaceDetailSheet(place: place, isLoggedIn: viewModel.isLoggedIn, placeToAdd: $placeToAdd)
                .presentationDetents([.fraction(0.35)])
                .presentationDragIndicator(.visible)
        }
        .sheet(item: $placeToAdd) { placeToSave in
            // REVISI: SEKARANG MEMAKAI OBYEK LANGSUNG BUKAN BOOLEAN, JADI DATA AMAN!
...
struct PlaceDetailSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            HStack(spacing: 16) {
                // REVISI: GAMBAR ASLI DARI FOURSQUARE (Jika ada)
                if let imgUrl = place.imageURL, let url = URL(string: imgUrl) {
                    AsyncImage(url: url) { phase in
                        if let image = phase.image {
                            image.resizable().scaledToFill().frame(width: 80, height: 80).clipShape(RoundedRectangle(cornerRadius: 12))
                        } else {
                            RoundedRectangle(cornerRadius: 12).fill(Color.gray.opacity(0.2)).frame(width: 80, height: 80).overlay(ProgressView())
                        }
                    }
                } else {
                    // Fallback
                    RoundedRectangle(cornerRadius: 12)
                        .fill(LinearGradient(colors: [Color.leapPrimary, Color.teal], startPoint: .topLeading, endPoint: .bottomTrailing))
            ...
            VStack(spacing: 8) {
                Button(action: {
                    let p = place // Simpan di memori lokal fungsi
                    dismiss()
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) {
                        placeToAdd = p // Eksekusi buka sheet kedua secara aman
                    }
                }) {
                    HStack {
