...
class TripDestinationService: TripDestinationServiceProtocol {
    ...
    func addPlaceToTrip(
        ...
        let existingPlans = try await firestoreRepo.fetchDayPlans(
            forTripID: tripID,
            userID: userID
        )

        for dayNum in selectedDays {
            if let targetPlanIndex = existingPlans.firstIndex(where: {
                $0.dayNumber == dayNum
            }) {
    ...
    func removePlaceFromTrip(
        ...
        if let index = existingPlans.firstIndex(where: {
            $0.dayNumber == dayNum
        }) {
    ...
    func calculateTimeline(
        ...
        guard
            let index = dayPlan.destinations.firstIndex(where: {
                $0.id == destination.id
            })
        else { return "" }
