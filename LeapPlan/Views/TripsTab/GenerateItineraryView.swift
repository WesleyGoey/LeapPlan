...
struct GenerateItineraryView: View {
        ...
        ZStack(alignment: .bottom) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(alignment: .leading, spacing: 24) {
                            ...
                            && !viewModel.autocompleteResults.isEmpty
                        {
                            VStack(spacing: 0) {
                                ForEach(
                                    ...
                                    if placeName
                                        != viewModel.autocompleteResults.last
                                    {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: 12) {
                            ForEach(viewModel.dailyPreferences, id: \.dayNumber)
                            { pref in dayTabButton(dayNumber: pref.dayNumber) }
                        }
                        .padding(.bottom, 8)
                    }

                    if let selectedIndex = viewModel.dailyPreferences
                        .firstIndex(where: {
                            $0.dayNumber == viewModel.selectedDayNumber
                        })
                    {
                        VStack(spacing: 16) {
                            stepperCard(
                                ...
                                subtitle: "Attractions per day",
                                value: $viewModel.dailyPreferences[
                                    selectedIndex
                                ].places
                            )
                        }
                ...
                .padding(.horizontal, 20).padding(.top, 20).padding(
                    .bottom,
                    120
                )
            }
            .simultaneousGesture(
                DragGesture().onChanged({ _ in isDestinationFocused = false })
            )
            ...
            Button {
                ...
                .foregroundColor(.white).frame(maxWidth: .infinity).padding(
                    .vertical,
                    18
                ).background(Color.leapPrimary).cornerRadius(16).shadow(
            ...
            if isGenerating {
                ...
                VStack(spacing: 16) {
                    ...
                    Text("Building your itinerary...").foregroundColor(.white)
                        .font(.headline)
                }
            }
        }
    ...
    private func dayTabButton(dayNumber: Int) -> some View {
        ...
        return Button {
            ...
            Text("Day \(dayNumber)").font(.subheadline.bold()).padding(
                .horizontal,
                20
            ).padding(.vertical, 10).background(
                isSelected ? Color.leapPrimary : Color.white
    ...
    private func stepperCard(
        ...
        HStack {
            Image(systemName: icon).font(.title2).foregroundColor(.gray).frame(
                width: 32
            )
            ...
            HStack(spacing: 16) {
                ...
                Text("\(value.wrappedValue)").font(.title3.bold()).frame(
                    width: 24
                )
                Button {
                    ...
                    Image(systemName: "plus").frame(width: 36, height: 36)
                        .background(Color.leapPrimary).clipShape(Circle())
                        .foregroundColor(.white)
                }
            }
        }.padding().background(Color.white).cornerRadius(16).shadow(
            color: .black.opacity(0.05),
            radius: 5,
            y: 2
        )
