...
class TripsViewModel: ObservableObject {
    ...
    func deleteTrip(tripID: String) {
        ...
        Task {
            do {
                try await tripRepository.deleteTrip(
                    tripID: tripID,
                    forUserID: userID
                )
    ...
    func updateTripDetails(
        ...
        do {
            ...
            let existingDayPlans = try await tripRepository.fetchDayPlans(
                forTripID: tripID,
                userID: userID
            )
            ...
            if existingDayPlans.count > totalDays {
                ...
                for plan in plansToDelete {
                    if let planID = plan.id {
                        try await tripRepository.deleteDayPlan(
                            planID: planID,
                            tripID: tripID,
                            userID: userID
                        )
                ...
                for i in (existingDayPlans.count + 1)...totalDays {
                    ...
                    try await tripRepository.saveDayPlan(
                        newPlan,
                        forTripID: tripID,
                        userID: userID
                    )
    ...
    // MARK: - LOGIKA CREATE MANUAL TRIP
        func createManualTrip(location: String, start: Date, end: Date) async throws -> Trip {
            let userID = activeUserID
            let title = "\(location) Trip"
            var newTrip = Trip(title: title, locationName: location, startDate: start, endDate: end, status: .upcoming, participantIDs: [userID], createdAt: Date(), createdBy: userID)
            newTrip.coverImageUrl = "https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?q=80&w=800&auto=format&fit=crop"
            
            // Buat Kerangka Hari (DayPlan) Kosong Sesuai Jumlah Hari
            let calendar = Calendar.current
            ...
            for i in 1...totalDays {
                if let newDate = calendar.date(byAdding: .day, value: i - 1, to: calendar.startOfDay(for: start)) {
                    emptyDays.append(DayPlan(id: UUID().uuidString, dayNumber: i, date: newDate, destinations: []))
                }
            }
            
            // Simpan Trip dan Hari Kosongnya ke Firebase
            try await tripRepository.saveGeneratedTripWithDayPlans(trip: newTrip, dayPlans: emptyDays, userID: userID)
            
            // Refresh UI List Depan
            await MainActor.run { self.loadUserTrips() }
