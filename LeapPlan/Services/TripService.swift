...
class TripService: TripServiceProtocol {
    ...
    func generateRandomItinerary(preferences: RandomTripPreferences) async throws -> [DayPlan] {
        ...
        var availablePlaces = try await foursquareService.fetchPlaces(
            near: preferences.locationName, categoryID: placeCategories, limit: 50
        )

        let invalidWords = ["toko", "store", "shop", "mart", "market", "pasar", "supermarket", "indomaret", "alfamart", "masjid", "vihara", "gereja", "pura", "temple", "shrine", "bank", "atm", "bca", "mandiri", "hotel", "penginapan", "kost", "resto", "cafe", "warung", "bakso", "soto", "nasi", "mie", "rs", "klinik", "hospital", "xxi", "cgv", "bioskop"]
        ...
        // Hapus duplikat manual (tanpa extension agar tidak error redeclaration)
        var addedDict = [String: Bool]()
        availablePlaces = availablePlaces.filter { addedDict.updateValue(true, forKey: $0.fsq_place_id) == nil }
